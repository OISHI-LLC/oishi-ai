...
PROMPT;
const HOMEPAGE_FACTS_PROMPT = <<<PROMPT
...
- サービス:
  - AI戦略コンサルティング: ビジネス課題のヒアリングからAI導入ロードマップの策定まで
  - カスタムAI開発: チャットボット・文書解析・画像認識・予測モデルなどの業務特化型AIシステム開発
  - AIエージェント開発: 情報収集・判断補助・タスク実行まで含む業務フロー自動化
  - 業務プロセス自動化: レポート作成・データ入力・メール対応などの定型業務自動化
  - AI研修・内製化支援: 生成AI活用研修と社内AIチーム立ち上げ支援
  - AI導入診断: AI導入余地の分析、投資対効果試算、レポート提供
  - 既存システムへのAI統合: API連携やプラグイン開発による既存環境の高度化
...
- 問い合わせ:
  - 初回の相談は無料
  - お問い合わせから最短3営業日で提案可能
  - 流れ: お問い合わせ → 無料ヒアリング（オンライン30〜60分程度）→ ご提案
  - お問い合わせページ: /contact/

- 注意:
  - 創業年、設立年、代表者名、従業員数、資本金、主要顧客、売上、取引実績件数などは、このホームページ記載情報には含まれていない
  - それらを聞かれたら「ホームページには明記されていません」と答える
PROMPT;
